...
func SetupRoutes(cfg *config.Config, notificationService *services.NotificationService, fileService *services.FileService) (*gin.Engine, *handlers.TelegramHandler, *services.SchedulerService) {
	router := gin.Default()
	...
	// 初始化Telegram处理器
	telegramHandler := handlers.NewTelegramHandler(cfg, notificationService, fileService, schedulerService)
