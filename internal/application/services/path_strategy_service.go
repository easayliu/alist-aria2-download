...
type PathStrategyService struct {
	...
	varExtractor     *VariableExtractor     // 变量提取器
	templateRenderer *TemplateRenderer      // 模板渲染器
	conflictDetector *ConflictDetector      // 冲突检测器
	mappingEngine    *PathMappingEngine     // 映射规则引擎（可选）
	pathAdapter      *PathAdapter           // 跨平台路径适配器
...
func NewPathStrategyService(
	...
	return &PathStrategyService{
		...
		pathValidator:    NewPathValidatorService(cfg),
		directoryMgr:     NewDirectoryManager(cfg),
		varExtractor:     NewVariableExtractor(),
		templateRenderer: NewTemplateRenderer(cfg.Download.PathConfig.Templates),
		conflictDetector: NewConflictDetector(cfg),
		mappingEngine:    mappingEngine,
		pathAdapter:      NewPathAdapter(),
